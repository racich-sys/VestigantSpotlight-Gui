# Vestigant Spotlight Investigator Help - V1.6.28

V1.6.28 fixes lookup-source reporting for Missing-from-FFS validation. Review `active_file_comparison_validation_checks_sample.csv` after the thin run. The expected result is that `reference_view_lookup_source_populated` changes from REVIEW to PASS. Missing rows remain lead-only and not deletion proof.

# Vestigant Spotlight Investigator Help - V1.6.28

V1.6.28 is a validation-hardening release after V1.6.26 active-comparison validation. Use the new validation samples before retiring additional guardrails:
