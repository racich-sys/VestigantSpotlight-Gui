...
$Wanted = @(
    ...
    "case_summary.csv",
    "case_summary.json",
    "three_database_layout_readiness.csv",
    ...
    "exports/ios_app_db_spotlight_enabled_summary.csv",
    "exports/ios_app_db_spotlight_schema_hits.csv",
    "exports/ios_app_db_spotlight_schema_hits_sample.csv",
    "exports/ios_app_db_spotlight_flag_candidates.csv",
    "exports/ios_app_db_spotlight_flag_candidates_sample.csv",
    "exports/ios_app_db_spotlight_row_candidates.csv",
    "exports/ios_app_db_spotlight_row_candidates_sample.csv",
    "exports/ios_app_database_record_summary.csv",
    "exports/ios_app_parsed_record_summary.csv",
    "exports/ios_app_parsed_records_sample.csv",
...
# Include portable Release runtime status when available so thin uploads show whether the copyable Release folder is complete.
try {
    $RepoRootForPortable = Split-Path -Parent $PSScriptRoot
    $ReleaseRootForPortable = Join-Path $RepoRootForPortable "build-msvc\Release"
    $PortableStatus = Join-Path $ReleaseRootForPortable "resources\portable_runtime_status.txt"
    if (Test-Path -LiteralPath $PortableStatus -PathType Leaf) {
        Copy-Item -LiteralPath $PortableStatus -Destination (Join-Path $UploadRoot "portable_runtime_status.txt") -Force
        $copied.Add("portable_runtime_status.txt") | Out-Null
    }
    $PortableManifest = Join-Path $ReleaseRootForPortable "resources\reader_tools\reader_tools_manifest.csv"
    if (Test-Path -LiteralPath $PortableManifest -PathType Leaf) {
        Copy-Item -LiteralPath $PortableManifest -Destination (Join-Path $UploadRoot "portable_release_reader_tools_manifest.csv") -Force
        $copied.Add("portable_release_reader_tools_manifest.csv") | Out-Null
    }
    $ProductionStatus = Join-Path $ReleaseRootForPortable "resources\production_readiness_status.txt"
    if (Test-Path -LiteralPath $ProductionStatus -PathType Leaf) {
        Copy-Item -LiteralPath $ProductionStatus -Destination (Join-Path $UploadRoot "production_readiness_status.txt") -Force
        $copied.Add("production_readiness_status.txt") | Out-Null
    }
    $PortableCheck = Join-Path $ReleaseRootForPortable "Check-PortableRuntime.ps1"
    if (Test-Path -LiteralPath $PortableCheck -PathType Leaf) {
        Copy-Item -LiteralPath $PortableCheck -Destination (Join-Path $UploadRoot "Check-PortableRuntime.ps1.txt") -Force
        $copied.Add("Check-PortableRuntime.ps1.txt") | Out-Null
    }
} catch {
    Write-Warning "Unable to copy portable Release runtime status into thin upload: $($_.Exception.Message)"
}
