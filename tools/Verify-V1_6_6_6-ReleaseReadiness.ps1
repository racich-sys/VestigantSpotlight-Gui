...
$build = Get-Content -LiteralPath (Join-Path $SourceRoot "scripts\Build-V1_6_6_6.ps1") -Raw
if ($build -notmatch "1\\.6\\.6\\.5") { throw "Build wrapper does not reference 1.6.6.6" }
if ($build -notmatch '\$version -notmatch "1\\.6\\.6\\.5"') { throw "Build wrapper CLI version validation is not pinned to 1.6.6.6" }
$ios = Join-Path $SourceRoot "scripts\Run-V1_6_6_6-iOS-CoreSpotlight-AndZip.ps1"
...
foreach ($needle in @("looksLikeAppleAbsoluteTimeNumeric", "IDENTITY_PROMOTION_SUPPRESSED=True", "KNOWLEDGEC_DEVICE_OR_APP_ACTIVITY", "KNOWLEDGEC_COMMUNICATION_INTENT")) {
  if ($iosParser -notmatch [regex]::Escape($needle)) { throw "iOS parser missing V1.6.6.6 identity/KnowledgeC guardrail: $needle" }
}
...
foreach ($needle in @("KNOWLEDGEC_COMMUNICATION_INTENT", "KNOWLEDGEC_DEVICE_OR_APP_ACTIVITY", "IDENTITY_PROMOTION_SUPPRESSED=True")) {
  if ($caseDb -notmatch [regex]::Escape($needle)) { throw "case_db.cpp missing V1.6.6.6 KnowledgeC classification guardrail: $needle" }
}
...
foreach ($needle in @("vw_ios_spotlight_comms_missing_from_ffs", "COMMUNICATION_PRESENT_IN_SPOTLIGHT_NOT_MATCHED_TO_NATIVE_APP_DB", "deletion, app removal, encryption/inaccessibility, parser coverage limits")) {
  if ($guiText -notmatch [regex]::Escape($needle)) { throw "win32_gui.cpp missing V1.6.6.6 GUI bootstrap communication/native-DB comparison view coverage: $needle" }
}
...
foreach ($needle in @("__native_core_probe_string_%", "native_probe_context_sample", "SPOTLIGHT_MESSAGE_OR_ATTACHMENT_TEXT_PROBE", "SPOTLIGHT_MAIL_OR_ACCOUNT_TEXT_PROBE", "10_string_probes")) {
  if ($caseDb -notmatch [regex]::Escape($needle)) { throw "case_db.cpp missing V1.6.6.6 native CoreSpotlight probe review coverage: $needle" }
}
...
foreach ($needle in @("coreProbeField", "looksLikeForensicReferenceValue", 'fieldLabel = "__native_core_probe_string"')) {
  if ($nativeParser -notmatch [regex]::Escape($needle)) { throw "native_storedb_parser.cpp missing V1.6.6.6 core probe text-context guard: $needle" }
}
...
foreach ($needle in @("source_field_name", "native_probe_context_count", "native_probe_context_sample")) {
  if ($registry -notmatch [regex]::Escape($needle)) { throw "view_registry.cpp missing V1.6.6.6 native probe review column: $needle" }
}
...
foreach ($needle in @("runKnowledgeCIdentitySuppressionSmokeTest", "runIosCoreProbeTextContextSmokeTest", "SPOTLIGHT_MESSAGE_OR_ATTACHMENT_TEXT_PROBE", "KNOWLEDGEC_DEVICE_OR_APP_ACTIVITY", "KNOWLEDGEC_COMMUNICATION_INTENT")) {
  if ($tests -notmatch [regex]::Escape($needle)) { throw "Self-test missing V1.6.6.6 KnowledgeC identity suppression coverage: $needle" }
}
...
foreach ($needle in @("THIN_PROFILE_DIRECT_IDENTITY_PIVOT", "THIN_PROFILE_DIRECT_CANDIDATE_SAMPLE", "THIN_PROFILE_NOT_EVALUATED")) {
  if ($exporter -notmatch [regex]::Escape($needle)) { throw "sqlite_exporter.cpp missing V1.6.6.6 thin export timeout guard: $needle" }
}
...
$forensic = Join-Path $SourceRoot "tools\Verify-V1_6_6_6-ForensicDirectives.ps1"
