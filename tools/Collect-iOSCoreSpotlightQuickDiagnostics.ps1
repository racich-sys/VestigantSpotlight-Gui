param(
  ...
  [string]$ReportRoot = "D:\Downloads\iOS_CoreSpotlight_QuickDiagnostics_V0_9_1",
  [string]$ReportZip = "D:\Downloads\iOS_CoreSpotlight_QuickDiagnostics_V0_9_1.zip",
  [string]$EvidenceRoot = "D:\Downloads\iOS_CoreSpotlight_MinimalEvidence_V0_9_1",
  [string]$EvidenceZip = "D:\Downloads\iOS_CoreSpotlight_MinimalEvidence_V0_9_1.zip",
...
function Get-DomainHint([string]$path) { $p=$path.ToLowerInvariant(); if($p.Contains('/library/sms/') -or $p.EndsWith('/sms.db')){return 'Messages'}; if($p.Contains('callhistory')){return 'CallHistory'}; if($p.Contains('whatsapp')){return 'WhatsApp'}; if($p.Contains('signal')){return 'Signal'}; if($p.Contains('telegram')){return 'Telegram'}; if($p.Contains('safari')){return 'Safari'}; if($p.Contains('chrome') -or $p.Contains('google')){return 'GoogleOrChrome'}; if($p.Contains('/mail/')){return 'Mail'}; if($p.Contains('/calendar/')){return 'Calendar'}; if($p.Contains('/addressbook/') -or $p.Contains('contacts')){return 'Contacts'}; if($p.Contains('fileprovider') -or $p.Contains('clouddocs') -or $p.Contains('mobile documents')){return 'FileProviderOrCloudDocs'}; return 'Other' }
...
function Get-DatabaseCategory([string]$path,[string]$name) { $p=$path.ToLowerInvariant(); $n=$name.ToLowerInvariant(); if($n -eq 'sms.db' -or $p.Contains('/library/sms/')){return @('APPLE_MESSAGES','Messages')}; if($p.Contains('callhistory') -or $n -like 'callhistory*'){return @('CALL_HISTORY','PhoneFaceTime')}; if($p.Contains('whatsapp') -or $n -eq 'chatstorage.sqlite'){return @('WHATSAPP','WhatsApp')}; if($p.Contains('signal')){return @('SIGNAL','Signal')}; if($p.Contains('telegram')){return @('TELEGRAM','Telegram')}; if($p.Contains('safari') -and ($n -like '*.db' -or $n -like '*.sqlite*')){return @('SAFARI_WEB','Safari')}; if(($p.Contains('chrome') -or $p.Contains('google')) -and ($n -like '*history*' -or $n -like '*.db' -or $n -like '*.sqlite*')){return @('CHROME_WEB','ChromeGoogle')}; if($p.Contains('webkit') -and ($n -like '*.db' -or $n -like '*.sqlite*')){return @('WEBKIT','WebKit')}; if($p.Contains('/mail/') -and ($n -like '*.db' -or $n -like '*.sqlite*')){return @('MAIL','Mail')}; if(($p.Contains('/calendar/') -or $n.Contains('calendar')) -and ($n -like '*.db' -or $n -like '*.sqlite*')){return @('CALENDAR','Calendar')}; if(($p.Contains('addressbook') -or $p.Contains('contacts')) -and ($n -like '*.db' -or $n -like '*.sqlite*')){return @('CONTACTS','Contacts')}; if($n -like '*.db' -or $n -like '*.sqlite' -or $n -like '*.sqlite3' -or $n -like '*.storedata'){return @('OTHER_SQLITE_OR_STORE_DATABASE','Other')}; return @('','') }
