...
if ((ReadText "VERSION") -notmatch '^1\.6\.17\s*$') { throw "VERSION is not $expected" }
if ((ReadText "VERSION.txt") -notmatch '^1\.6\.17\s*$') { throw "VERSION.txt is not $expected" }
if ((ReadText "CMakeLists.txt") -notmatch 'VERSION 1\.6\.17') { throw "CMakeLists.txt project version is not $expected" }
$build = ReadText "scripts\Build-V1_6_17.ps1"
if ($build -notmatch [regex]::Escape('Verify-V1_6_17-ReleaseReadiness.ps1')) { throw "Build wrapper does not call current release-readiness script" }
$cliVersionRegexPin = [regex]::Escape('1\.6\.17')
$cliVersionLiteralPin = [regex]::Escape('1.6.17')
...
foreach ($script in @("scripts\Run-V1_6_17-iOS-CoreSpotlight-AndZip.ps1", "scripts\Run-V1_6_17-iOS-Production-AndZip.ps1", "scripts\Launch-V1_6_17-GUI.ps1")) {
  $txt = ReadText $script
  if ($txt -match $staleRegex) { throw "$script contains stale previous-version reference" }
}
$prod = ReadText "scripts\Run-V1_6_17-iOS-Production-AndZip.ps1"
...
foreach ($needle in @('requiredUploadSamples', 'artifacts_sample.csv', 'raw_key_values_sample.csv', 'timeline_events_sample.csv', 'usage_evidence_sample.csv', 'missing required bounded validation samples')) {
  if ($runner -notmatch [regex]::Escape($needle)) { throw "iOS focused runner missing V1.6.17 upload-sample ZIP validation marker: $needle" }
}
...
foreach ($needle in @('KNOWLEDGEC_COMMUNICATION_INTENT', 'KNOWLEDGEC_DEVICE_OR_APP_ACTIVITY', 'IDENTITY_PROMOTION_SUPPRESSED=True', 'vw_ios_spotlight_comms_missing_from_ffs', 'vw_ios_production_readiness_summary', 'PRODUCTION_READY_HASH_RECORDED')) {
  if ($caseDb -notmatch [regex]::Escape($needle)) { throw "case_db.cpp missing V1.6.17 required view/guardrail: $needle" }
}
...
foreach ($needle in @('__native_probe_', '__native_core_probe_string_', '_candidate_', 'incorrectly inserted into raw_date_candidates')) {
  if ($nativeParser -notmatch [regex]::Escape($needle)) { throw "native_storedb_parser.cpp missing V1.6.17 date-candidate alias guard: $needle" }
}
foreach ($needle in @('extractFirstEmailCandidate', 'isLikelyEmailCandidate', 'isEmailLocalPartChar', 'isEmailDomainChar')) {
  if ($nativeParser -notmatch [regex]::Escape($needle)) { throw "native_storedb_parser.cpp missing V1.6.17 semantic email alias precision marker: $needle" }
}
foreach ($needle in @('__native_probe_email_candidate_%', "v NOT LIKE '%file:%'", "v NOT LIKE '%/%'", "v NOT LIKE '% %'")) {
  if ($caseDb -notmatch [regex]::Escape($needle)) { throw "case_db.cpp missing V1.6.17 iOS email-category precision marker: $needle" }
}
foreach ($needle in @('__native_probe_email_candidate_%', "v NOT LIKE '%file:%'", "v NOT LIKE '%/%'", "v NOT LIKE '% %'")) {
  if ($exporter -notmatch [regex]::Escape($needle)) { throw "sqlite_exporter.cpp missing V1.6.17 iOS email-category export precision marker: $needle" }
}
foreach ($needle in @('lower.compare(end, 8, "https://")', 'lower.compare(end, 7, "http://")', 'lower.compare(end, 7, "file://")')) {
  if ($nativeParser -notmatch [regex]::Escape($needle)) { throw "native_storedb_parser.cpp missing V1.6.17 URL/path concatenation guard marker: $needle" }
}
foreach ($needle in @('isUsefulDerivedBasenameCandidate', 'highBytes', 'filenameSafe', 'punctuationNoise', 'Keep raw __native_core_probe_string_## evidence untouched')) {
  if ($nativeParser -notmatch [regex]::Escape($needle)) { throw "native_storedb_parser.cpp missing V1.6.17 basename precision marker: $needle" }
}
...
foreach ($needle in @('temp_parent_inode_path_candidates', 'ANCESTOR_PATH_PLUS_CHILD_CHAIN', 'RELATIVE_PARENT_INODE_NAME_CHAIN', 'PARENT_INODE_CHAIN_RECONSTRUCTION', 'parent_inode_links_with_new_reconstructed_path', 'parent_inode_links_with_existing_path_context')) {
  if ($enrich -notmatch [regex]::Escape($needle)) { throw "sqlite_enrichment.cpp missing V1.6.17 parent-path reconstruction marker: $needle" }
}
if ($appRunner -notmatch [regex]::Escape('aff4_apfs_staged_storev2_path_reconstruction_sample.csv')) { throw "app_runner.cpp missing V1.6.17 AFF4 path reconstruction sample export" }
if ($appRunner -notmatch [regex]::Escape('aff4_apfs_staged_storev2_path_reconstruction_metrics_sample.csv')) { throw "app_runner.cpp missing V1.6.17 AFF4 path reconstruction metrics sample export" }
if ($uploadTool -notmatch [regex]::Escape('aff4_apfs_staged_storev2_path_reconstruction_sample.csv')) { throw "upload tool missing V1.6.17 AFF4 path reconstruction sample" }
if ($uploadTool -notmatch [regex]::Escape('aff4_apfs_staged_storev2_path_reconstruction_metrics_sample.csv')) { throw "upload tool missing V1.6.17 AFF4 path reconstruction metrics sample" }
...
foreach ($needle in @('runKnowledgeCIdentitySuppressionSmokeTest', 'runIosCoreProbeTextContextSmokeTest', 'runIosProductionReadinessSmokeTest', 'runParentInodePathReconstructionSmokeTest', 'vw_ios_production_readiness_summary')) {
  if ($tests -notmatch [regex]::Escape($needle)) { throw "Self-test missing V1.6.17 coverage: $needle" }
}
$exporter = ReadText "src\export_sql\sqlite_exporter.cpp"
foreach ($needle in @('ios_production_readiness_summary.csv', 'FORCE_CONTAINER_HASH_REQUESTED', 'THIN_PROFILE_DIRECT_IDENTITY_PIVOT', 'THIN_PROFILE_DIRECT_CANDIDATE_SAMPLE', 'THIN_PROFILE_NOT_EVALUATED')) {
  if ($exporter -notmatch [regex]::Escape($needle)) { throw "sqlite_exporter.cpp missing V1.6.17 production/thin guard: $needle" }
}
...
Write-Host $raw
$forensic = Join-Path $SourceRoot "tools\Verify-V1_6_17-ForensicDirectives.ps1"
