...
- Hotfixes MSVC GUI build failure caused by helper functions moved into `gui_export_worker.cpp` without visible definitions.
- Makes GUI multi-view export worker self-contained for folder creation, path joining, and export filename sanitization.
- Adds parser-side rejection for Apple CFAbsoluteTime-like decimal numeric values before phone identity promotion.
- Makes KnowledgeC/CoreDuet identity promotion stream-aware so device/app state streams such as `/display/isBacklit` are not promoted as identity evidence.
- Adds release-readiness checks and `ai_context.md` guardrails for build wrapper presence and helper visibility after modularization.

## V1_6_5

- Added GUI/CLI option to suppress end-of-run CSV review exports for full GUI ingestion workflows.
- Added `RunOptions::suppressCsvExports`, CLI `--no-csv-exports`, GUI `SQLite only (no CSV dump)` checkbox, and export-disabled marker files.
- Updated release readiness checks to require the iOS wrapper expose `-NoCsvExports`.
