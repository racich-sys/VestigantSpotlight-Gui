...
MacOS Store-V2 external dbStr map loading fix. The parser now loads adjacent dbStr maps for macOS Store-V2 when the header dictionary pointers are zero and the sidecar map files exist.

# V1.6.35

MacOS Store-V2 path enrichment hotfix. V1.6.35 promotes `__native_probe_file_path_candidate_%` values to artifact display/path fields when raw record headers lack usable names or paths.

# 1.6.35

V1.6.35 follows the V1.6.32 macOS zipped Spotlight thin review. The source parsed successfully, but enrichment spent several minutes in a no-op parent-inode path apply step. This version skips that UPDATE when `new_reconstructed_paths=0`.
...
## V1.6.35
V1.6.35 follows V1.6.27 thin validation. The thin showed active comparison and CoreDuet validation exports working, but the lookup-source validation check remained REVIEW. V1.6.35 updates the later GUI/review Missing-from-FFS view definition and the materialized candidate reason builder to preserve a nonblank lookup source.

# Consolidated Version History

## V1.6.35
V1.6.35 validates the V1.6.26 active-comparison work by adding reconciliation checks and bounded validation samples for active comparison and CoreDuet interactionC. The V1.6.26 thin showed active comparison completed with 7,766 materialized reference candidates; V1.6.35 adds validation surfaces to confirm those rows reconcile to summary and reference views and preserve lead-only wording.
...
V1.6.35 validates V1.6.29.4 and applies additional source-grounded hardening: checked-artifact mutex coverage, stale review-query detach instead of UI-thread join, export worker handle accumulation reduction, length-aware SQLite CSV export, and APFS NXSB block-size rejection before allocation/use.
