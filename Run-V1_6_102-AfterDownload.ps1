...
if (!(Test-Path -LiteralPath $ZipPath -PathType Leaf)) {
  throw "Required source ZIP not found: $ZipPath. Download VestigantSpotlightInv_V1_6_102.zip to D:\Downloads or pass -ZipPath."
}

# V1.6.102 intentionally ignores any sibling/stale BuildAndRun wrapper in D:\Downloads.
# The authoritative script is extracted fresh from the ZIP so old same-name wrappers cannot reintroduce old flags.
$bootstrapRoot = Join-Path $DownloadRoot "_VestigantSpotlightInv_V1_6_102_Bootstrap"
$bootstrapSourceRoot = Join-Path $bootstrapRoot "VestigantSpotlightInv_V1_6_102"
$buildAndRun = Join-Path $bootstrapSourceRoot "BuildAndRun-V1_6_102-FromDownloadedZip.ps1"

Write-Host "Vestigant Spotlight V1.6.102 one-click build/run wrapper"
...
if ($exitCode -ne 0) { throw "V1.6.102 one-click build/run workflow failed with exit code $exitCode" }
Write-Host "V1.6.102 one-click build/run workflow completed."
