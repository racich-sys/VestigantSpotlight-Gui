...
$ReleaseReadiness = Join-Path $SourceRoot "tools\Verify-V1_6_115-ReleaseReadiness.ps1"
...
if (Test-Path -LiteralPath $ProductionVerify -PathType Leaf) {
  $oldEap = $ErrorActionPreference
  $ErrorActionPreference = "Continue"
  $productionOutput = & powershell -ExecutionPolicy Bypass -File $ProductionVerify -SourceRoot $SourceRoot -ExpectedVersion "1.6.115" 2>&1
...
if (Test-Path -LiteralPath $PortableExport -PathType Leaf) {
  $portableZip = "D:\Downloads\VestigantSpotlightPortable_V1_6_115.zip"
  try {
