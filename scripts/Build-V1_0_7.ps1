param(
  [string]$ZipPath = "D:\Downloads\VestigantSpotlightInv_V1_0_7.zip",
  [string]$SourceRoot = "T:\VestigantSpotlightInv_V1_0_7",
  [string]$BuildLog = "D:\Downloads\V1_0_7_build.log",
  [switch]$CleanExtract
)
...
if ($version -notmatch "1\.0\.7") { throw "Unexpected CLI version after build: $version" }
Write-Host $version
