param(
  [string]$ZipPath = "D:\Downloads\VestigantSpotlightInv_V1_6_18.zip",
  [string]$SourceRoot = "T:\VestigantSpotlightInv_V1_6_18",
  [string]$BuildLog = "D:\Downloads\V1_6_18_build.log",
  [switch]$CleanExtract
)
...
$ReleaseReadiness = Join-Path $SourceRoot "tools\Verify-V1_6_18-ReleaseReadiness.ps1"
...
if ($version -notmatch "1\.6\.18") { throw "Unexpected CLI version after build: $version" }
Write-Host $version
