param(
  [string]$ZipPath = "D:\Downloads\VestigantSpotlightInv_V1_2_0.zip",
  [string]$SourceRoot = "T:\VestigantSpotlightInv_V1_2_0",
  [string]$BuildLog = "D:\Downloads\V1_2_0_build.log",
  [switch]$CleanExtract
)
...
if ($version -notmatch "1\.2\.0") { throw "Unexpected CLI version after build: $version" }
Write-Host $version
