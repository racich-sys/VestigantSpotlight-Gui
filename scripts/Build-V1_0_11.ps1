param(
  [string]$ZipPath = "D:\Downloads\VestigantSpotlightInv_V1_0_11.zip",
  [string]$SourceRoot = "T:\VestigantSpotlightInv_V1_0_11",
  [string]$BuildLog = "D:\Downloads\V1_0_11_build.log",
  [switch]$CleanExtract
)
...
if ($version -notmatch "1\.0\.11") { throw "Unexpected CLI version after build: $version" }
Write-Host $version
