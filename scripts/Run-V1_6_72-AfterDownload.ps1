...
$buildAndRun = Join-Path $DownloadRoot "BuildAndRun-V1_6_72-FromDownloadedZip.ps1"
if (!(Test-Path -LiteralPath $buildAndRun -PathType Leaf)) {
  $sibling = Join-Path (Split-Path -Parent $MyInvocation.MyCommand.Path) "BuildAndRun-V1_6_72-FromDownloadedZip.ps1"
  if (Test-Path -LiteralPath $sibling -PathType Leaf) { $buildAndRun = $sibling }
}
if (!(Test-Path -LiteralPath $ZipPath -PathType Leaf)) {
  throw "Required source ZIP not found: $ZipPath. Download VestigantSpotlightInv_V1_6_72.zip to D:\Downloads or pass -ZipPath."
}
if (!(Test-Path -LiteralPath $buildAndRun -PathType Leaf)) {
  $bootstrapRoot = Join-Path $DownloadRoot "_VestigantSpotlightInv_V1_6_72_Bootstrap"
  $bootstrapSourceRoot = Join-Path $bootstrapRoot "VestigantSpotlightInv_V1_6_72"
  $bootstrapBuildAndRun = Join-Path $bootstrapSourceRoot "BuildAndRun-V1_6_72-FromDownloadedZip.ps1"
...
Write-Host "Vestigant Spotlight V1.6.72 one-click build/run wrapper"
...
if ($exitCode -ne 0) { throw "V1.6.72 one-click build/run workflow failed with exit code $exitCode" }
Write-Host "V1.6.72 one-click build/run workflow completed."
