param(
  [string]$ZipPath = "D:\Downloads\VestigantSpotlightInv_V1_3_0.zip",
  [string]$SourceRoot = "T:\VestigantSpotlightInv_V1_3_0",
  [string]$BuildLog = "D:\Downloads\V1_3_0_build.log",
  [switch]$CleanExtract
)
...
if ($version -notmatch "1\.3\.0") { throw "Unexpected CLI version after build: $version" }
Write-Host $version
