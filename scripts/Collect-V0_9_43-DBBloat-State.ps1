param(
  ...
  [string]$OutZip = "D:\Downloads\Upload_State_V0_9_43_DB_Bloat_Stopped_Check.zip",
  [switch]$StopVestigant
)

$ErrorActionPreference = "Continue"
$work = Join-Path $env:TEMP ("Vestigant_V0_9_43_State_" + (Get-Date -Format "yyyyMMdd_HHmmss"))
New-Item -ItemType Directory -Force -Path $work | Out-Null

$summary = Join-Path $work "00_summary.txt"
@(
  "Vestigant Spotlight V0_9_43 DB/WAL/no-writes state collection",
