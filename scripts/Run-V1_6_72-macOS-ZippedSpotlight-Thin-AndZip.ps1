param(
  [string]$SourceRoot = "T:\VestigantSpotlightInv_V1_6_72",
  [string]$ZipInput = "E:\test second\Spotlight\Spotlight-V100.zip",
  [string]$CaseRoot = "Q:\SpotlightCase\TestMacOS_ZippedSpotlight_Thin_V1_6_72",
  [string]$ThinZipPath = "D:\Downloads\Upload_Thin_MacOS_ZippedSpotlight_V1_6_72.zip",
  [string]$UploadWorkRoot = "D:\Downloads\Upload_Thin_MacOS_ZippedSpotlight_V1_6_72_UploadWork",
  [string]$RunLog = "D:\Downloads\V1_6_72_macos_zipped_spotlight_thin.log",
...
function Invoke-NativeLogged {
  ...
  $stdout = Join-Path (Split-Path -Parent $LogPath) "V1_6_72_macos_zipped_spotlight_stdout.txt"
  $stderr = Join-Path (Split-Path -Parent $LogPath) "V1_6_72_macos_zipped_spotlight_stderr.txt"
...
Write-LogLine "Starting V1.6.72 macOS zipped Spotlight thin test"
...
if (!(Test-Path -LiteralPath $cli)) { throw "CLI executable not found. Build V1.6.72 first: $cli" }
...
$args = @(
  ...
  "--input", $ZipInput,
  "--out", $CaseRoot,
  "--case-name", "MacOS_ZippedSpotlight_Thin_V1_6_72",
  "--investigator", "V1.6.72 macOS zipped Spotlight thin validation",
...
Copy-FileIfExists -Path (Join-Path (Split-Path -Parent $RunLog) "V1_6_72_macos_zipped_spotlight_stdout.txt") -DestinationDirectory $UploadWorkRoot
Copy-FileIfExists -Path (Join-Path (Split-Path -Parent $RunLog) "V1_6_72_macos_zipped_spotlight_stderr.txt") -DestinationDirectory $UploadWorkRoot

$summary = Join-Path $UploadWorkRoot "WRAPPER_SUMMARY.txt"
@(
  "wrapper=Run-V1_6_72-macOS-ZippedSpotlight-Thin-AndZip.ps1",
