param(
  [string]$CaseRoot = "Q:\SpotlightCase\TestiOS_WhatsApp_V0_9_20_ReusedCache",
  [string]$OutZip = "D:\Downloads\Upload_State_V0_9_20_DB_Bloat_Stopped_Check.zip",
  [switch]$StopVestigant
)

$ErrorActionPreference = "Continue"
$work = Join-Path $env:TEMP ("Vestigant_V0_9_20_State_" + (Get-Date -Format "yyyyMMdd_HHmmss"))
New-Item -ItemType Directory -Force -Path $work | Out-Null

$summary = Join-Path $work "00_summary.txt"
@(
  "Vestigant Spotlight V0_9_20 DB/WAL bloat state collection",
...
Get-Process | Where-Object { $_.ProcessName -like "VestigantSpotlight*" } |
  Select-Object Id,ProcessName,CPU,StartTime,Path |
  Format-Table -AutoSize | Out-String | Set-Content -LiteralPath (Join-Path $work "01_processes_before_stop.txt") -Encoding UTF8
...
Get-Process | Where-Object { $_.ProcessName -like "VestigantSpotlight*" } |
  Select-Object Id,ProcessName,CPU,StartTime,Path |
  Format-Table -AutoSize | Out-String | Set-Content -LiteralPath (Join-Path $work "02_processes_after_stop_sample.txt") -Encoding UTF8
...
Compress-Archive -LiteralPath (Join-Path $work "*") -DestinationPath $OutZip -Force
