param(
  ...
  [string]$ThinCaseRoot = "Q:\SpotlightCase\TestIOS_CoreSpotlight_V1_6_119",
  [string]$ThinZipPath = "D:\Downloads\Upload_Thin_iOS_CoreSpotlight_V1_6_119.zip",
  [string]$ChatUploadBundlePath = "D:\Downloads\Upload_To_Chat_V1_6_119_Results.zip",
  [string]$ChatUploadBundleWorkRoot = "D:\Downloads\Upload_To_Chat_V1_6_119_Results_Work",
  ...
  [string]$Aff4CaseRoot = "Q:\SpotlightCase\TestMacOS_AFF4_V1_6_119",
  [string]$ReaderToolsRoot = "",
  [string]$Aff4ZipPath = "D:\Downloads\Upload_Thin_MacOS_AFF4_V1_6_119.zip",
...
if ($UseFastLocalCaseRoot -and $SelectedWorkflow -eq "IOSCoreSpotlightThin") {
  $ThinCaseRoot = Join-Path $FastLocalRoot "TestIOS_CoreSpotlight_V1_6_119"
  Write-Host "Using fast local thin case root: $ThinCaseRoot"
}

Write-Host "Vestigant Spotlight V1.6.119 automated workflow"
...
if ($SelectedWorkflow -eq "AFF4Probe") {
  ...
  $summaryPath = "D:\Downloads\V1_6_119_AFF4_WRAPPER_RUN_SUMMARY.txt"
  @(
    "Vestigant Spotlight V1.6.119 automated AFF4 wrapper run summary",
  ...
  if ($runnerExit -ne 0) {
    $emergencyZip = "D:\Downloads\Upload_Thin_MacOS_AFF4_V1_6_119_FAILED_WRAPPER_RESCUE.zip"
    $emergencyWork = "D:\Downloads\Upload_Thin_MacOS_AFF4_V1_6_119_FAILED_WRAPPER_RESCUE_Work"
    try {
  ...
  $ExternalVolumeAudit = Join-Path $SourceRoot "tools\Verify-SpotlightExternalVolumePrecision.ps1"
  if (Test-Path -LiteralPath $ExternalVolumeAudit) {
    Write-Host "Running Spotlight external-volume precision audit: $ExternalVolumeAudit"
    powershell -ExecutionPolicy Bypass -File $ExternalVolumeAudit -ZipPath $Aff4ZipPath
    if ($LASTEXITCODE -ne 0) { throw "Spotlight external-volume precision audit failed with exit code $LASTEXITCODE. Upload ZIP: $Aff4ZipPath" }
  }
...
$ThinScript = Join-Path $SourceRoot "scripts\Run-V1_6_119-iOS-CoreSpotlight-AndZip.ps1"
...
Copy-IfExistsToBundle -Path $BuildLog -Name "V1_6_119_build.log"
Copy-IfExistsToBundle -Path $ThinZipPath -Name "Upload_Thin_iOS_CoreSpotlight_V1_6_119.zip"
...
@"
Vestigant Spotlight V1.6.119 automated wrapper run summary
...
@"
Vestigant Spotlight V1.6.119 automated chat upload bundle
Created: $(Get-Date -Format o)
Build log: $BuildLog
