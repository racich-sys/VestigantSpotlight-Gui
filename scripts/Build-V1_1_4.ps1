param(
  [string]$ZipPath = "D:\Downloads\VestigantSpotlightInv_V1_1_4.zip",
  [string]$SourceRoot = "T:\VestigantSpotlightInv_V1_1_4",
  [string]$BuildLog = "D:\Downloads\V1_1_4_build.log",
  [switch]$CleanExtract
)
...
if ($version -notmatch "1\.1\.4") { throw "Unexpected CLI version after build: $version" }
Write-Host $version
