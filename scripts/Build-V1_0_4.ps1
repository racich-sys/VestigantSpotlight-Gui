param(
  [string]$ZipPath = "D:\Downloads\VestigantSpotlightInv_V1_0_4.zip",
  [string]$SourceRoot = "T:\VestigantSpotlightInv_V1_0_4",
  [string]$BuildLog = "D:\Downloads\V1_0_4_build.log",
  [switch]$CleanExtract
)
...
if ($version -notmatch "1\.0\.4") { throw "Unexpected CLI version after build: $version" }
Write-Host $version
