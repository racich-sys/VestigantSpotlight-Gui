param(
  ...
  [string]$CaseRoot = "Q:\SpotlightCase\TestMacOS_AFF4_V1_3_5",
...
if (!$SkipExternalCompare -and (Test-Path -LiteralPath $ExternalSpotlightRoot)) {
  ...
  $args.ExternalCompareOutRoot = "D:\Downloads\Upload_Thin_MacOS_AFF4_V1_3_5_ExternalCompare"
...
& $runner @args
if ($LASTEXITCODE -ne 0) { throw "AFF4/APFS V1.3.5 probe wrapper failed with exit code $LASTEXITCODE" }
