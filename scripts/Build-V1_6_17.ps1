param(
  [string]$ZipPath = "D:\Downloads\VestigantSpotlightInv_V1_6_17.zip",
  [string]$SourceRoot = "T:\VestigantSpotlightInv_V1_6_17",
  [string]$BuildLog = "D:\Downloads\V1_6_17_build.log",
  [switch]$CleanExtract
)
...
$ReleaseReadiness = Join-Path $SourceRoot "tools\Verify-V1_6_17-ReleaseReadiness.ps1"
...
if ($version -notmatch "1\.6\.17") { throw "Unexpected CLI version after build: $version" }
Write-Host $version
