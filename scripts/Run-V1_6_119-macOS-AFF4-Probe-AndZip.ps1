param(
  ...
  [string]$CaseRoot = "Q:\SpotlightCase\TestMacOS_AFF4_V1_6_119",
  [string]$ReaderToolsRoot = "",
  [string]$ExternalSpotlightRoot = "",
  [string]$ExternalCompareOutRoot = "D:\Downloads\Upload_Thin_MacOS_AFF4_V1_6_119_ExternalCompare",
  [string]$UploadWorkRoot = "D:\Downloads\Upload_Thin_MacOS_AFF4_V1_6_119_UploadWork",
...
if ($LASTEXITCODE -ne 0) { throw "AFF4/APFS V1.6.119 probe wrapper failed with exit code $LASTEXITCODE. Review case folder: $CaseRoot" }
...
if ($RequireStoreV2ValidationOutputs) {
  ...
  try {
    ...
    foreach ($name in @(
      ...
      "aff4_apfs_staged_storev2_parse_selection_coverage.csv",
      "aff4_apfs_staged_storev2_parse_selection_coverage_summary.json",
