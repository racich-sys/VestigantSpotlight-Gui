param(
  ...
  [string]$CaseRoot = "Q:\SpotlightCase\TestMacOS_AFF4_V1_6_3",
  [string]$ReaderToolsRoot = "T:\VestigantReaderTools\aff4-cpp-lite",
  [string]$ZipPath = "D:\Downloads\Upload_Thin_MacOS_AFF4_V1_6_3.zip",
...
if (!$SkipExternalCompare -and (Test-Path -LiteralPath $ExternalSpotlightRoot)) {
  ...
  $args.ExternalCompareOutRoot = "D:\Downloads\Upload_Thin_MacOS_AFF4_V1_6_3_ExternalCompare"
...
& $runner @args
if ($LASTEXITCODE -ne 0) { throw "AFF4/APFS V1.6.3 probe wrapper failed with exit code $LASTEXITCODE" }
