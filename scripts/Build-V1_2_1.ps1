param(
  [string]$ZipPath = "D:\Downloads\VestigantSpotlightInv_V1_2_1.zip",
  [string]$SourceRoot = "T:\VestigantSpotlightInv_V1_2_1",
  [string]$BuildLog = "D:\Downloads\V1_2_1_build.log",
  [switch]$CleanExtract
)
...
if ($version -notmatch "1\.2\.1") { throw "Unexpected CLI version after build: $version" }
Write-Host $version
