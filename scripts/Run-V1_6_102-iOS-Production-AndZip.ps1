param(
  ...
  [string]$CaseRoot = "Q:\SpotlightCase\ProductionIOS_CoreSpotlight_V1_6_102",
  [string]$ZipPath = "D:\Downloads\Upload_Production_iOS_CoreSpotlight_V1_6_102.zip",
  [switch]$CleanOut,
  [switch]$NoClipboardOrExplorer,
  [switch]$MaterializeIosSupportDb,
  [string]$ExternalSourceSha256 = "",
  [string]$ExternalSourceHashNote = "",
  [switch]$UseExternalSourceHashOnly
)
...
if ($UseExternalSourceHashOnly) {
  if ([string]::IsNullOrWhiteSpace($ExternalSourceSha256)) { throw "-UseExternalSourceHashOnly requires -ExternalSourceSha256." }
} else {
  $args.ForceContainerHash = $true
}
...
if ($LASTEXITCODE -ne 0) {
  ...
  throw "iOS CoreSpotlight V1.6.102 production wrapper failed with exit code $LASTEXITCODE"
}
