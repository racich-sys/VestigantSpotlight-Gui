param(
    [string]$Aff4Input = "",
    [string]$Out = "Q:\SpotlightCase\TestMacOS_AFF4_V1_6_119",
    [string]$ReaderToolsRoot = "",
    [string]$ZipPath = "D:\Downloads\Upload_Thin_MacOS_AFF4_V1_6_119.zip",
...
if ($PressureTestMode) { Write-Host "Pressure Test / Triage mode requested: CLI will receive --pressure-test and --skip-container-hash." }
...
if ($DecodeCoreNativeValues -and -not ($FullNativeValues -or $PressureTestMode)) { $args += "--decode-core-native-values" }
if ($FullNativeValues -or $PressureTestMode) { $args += "--experimental-full-native-values" }
...
$expectedRunOutputs = @(
    ...
    "aff4_apfs_staged_storev2_parser_probe.csv", "aff4_apfs_staged_storev2_parse_selection_coverage.csv", "aff4_apfs_staged_storev2_parse_selection_coverage_summary.json",
