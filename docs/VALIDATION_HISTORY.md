...
## V0_9_17 validation note

Sandbox validation completed: C++ syntax-only checks passed for parser/database/export/app files; Linux CMake build completed; CLI version returned `Vestigant Spotlight v0.9.17`; Linux self-test passed; SQL raw-string scan found no nested `R"SQL` markers and no oversized SQL segment above 7,000 characters. Windows/MSVC build, Win32 GUI launch, and full reused-cache runtime remain user-side validation items.
...
# Vestigant Spotlight Validation History

This consolidated validation history replaces loose root validation note fragments in production packages.

## V0_8_77_BUILD_VALIDATION_NOTES.txt

Vestigant Spotlight V0_8_77 build validation notes

Changes:
- Fixed Win32 GUI C2026 string literal overflow by splitting the large SQL raw string in src/gui/win32_gui.cpp.
- Suppressed app_runner.cpp fullScan unused warning.
- Added user manual, quick start, and troubleshooting docs.
- Hid legacy V7 import from normal CLI usage text; legacy code remains isolated for compatibility pending dependency removal.
- Reduced build-log V7 visibility by compiling the legacy importer as a generic legacy compatibility component and not dumping the linker response object list.

Validation performed in sandbox:
- CMake configure completed.
- Linux CMake build completed.
- VestigantSpotlightCli --version returned Vestigant Spotlight v0.8.77.
- VestigantSpotlightTests passed.
- SQL raw string block sizes checked; largest block after split is under the prior MSVC C2026 risk range.

Not validated in sandbox:
- Windows/MSVC build.
- Win32 GUI compile/link.
- Full GUI iOS run.
- Full APFS/AFF4 run.

## V0_8_79_BUILD_VALIDATION_NOTES.txt

Vestigant Spotlight V0_8_79 build validation notes

Purpose:
- Correct the V0_8_78 iOS GUI ZIP-intake path that completed successfully but discovered zero store.db candidates.
- Preserve CLI/GUI parity for iOS focused CoreSpotlight extraction and all-valid-store parsing.
- Reduce build-script noise by ensuring build_windows_msvc.bat is CRLF encoded.

Uploaded V0_8_78 GUI iOS finding:
- Run completed with complete_success, but parser received stores=0.
- V0_8_78 logs show generic ZIP staging, not the iOS focused ZIP extraction route.
- case_summary.json/store_inventory.csv/store_selection.csv all confirmed zero iOS parser candidates.
- ios_input_store_entry_inventory.csv was not present in the upload bundle, which means the focused iOS ZIP extractor did not execute for the GUI run.
...
V0_8_79 changes:
1. Auto-profile ZIP staging now first attempts focused iOS CoreSpotlight extraction.
2. If focused extraction finds store.db/.store.db rows, the run is promoted to iOS/CoreSpotlight before store discovery and parser selection.
3. If focused extraction finds no iOS store rows, Auto profile falls back to generic ZIP extraction for macOS/folder-style Spotlight ZIPs.
4. Explicit iOS profile still uses focused extraction and reports an explicit empty iOS result if no store rows are found.
5. build_windows_msvc.bat rewritten with CRLF endings to avoid intermittent batch-label lookup messages.
6. Release notes/manual quick-start/troubleshooting docs updated for V0_8_79.

Validation performed in sandbox:
- app_runner.cpp syntax check with g++ -fsyntax-only passed.
- Full Linux CMake build was attempted but timed out while compiling the large app_runner.cpp translation unit; no compile errors were observed before timeout.
- Windows/MSVC build and GUI runtime still require user environment.

Expected V0_8_79 GUI iOS success indicators:
- run_status.txt should include stage_zip_source and, if Auto profile finds iOS stores, zip_profile_auto_promoted_ios.
- logs/ios_focused_zip_extract.log should exist.
- ios_input_store_entry_inventory.csv should exist and contain store.db/.store.db rows when the ZIP contains CoreSpotlight stores.
- store_inventory.csv and store_selection.csv should show nonzero database candidates.
- case_summary.json should show nonzero raw_records/artifacts/timeline if parsing succeeds.

## V0_8_80_BUILD_VALIDATION_NOTES.txt

Vestigant Spotlight V0_8_80 build/validation notes

Reviewed uploaded V0_8_79 iOS GUI attempt:
- last_stage.txt shows failed_exception: iOS focused ZIP extraction failed.
- run_status.txt shows the run reached source_probe_signature_scan, source_probe_signature_complete, and stage_zip_source with iOS focused extraction.
- VestigantSpotlight.log shows app_version=0.8.79 and attempted focused iOS extraction to EvidenceStaging/zip_source/extracted.
- ios_focused_zip_extract.log shows matched_entries=0 and store_db_entries=0, then throws no iOS CoreSpotlight/BundleInfo entries were extracted.

Issue classification:
- V0_8_78 issue was generic ZIP staging / profile routing.
- V0_8_79 moved past that and reached focused iOS extraction.
- V0_8_79 still failed because the generated extractor used .NET System.IO.Compression and did not find the CoreSpotlight paths in the large forensic Zip64 iOS FFS ZIP.
- Earlier successful focused extraction tooling used 7-Zip recursive include patterns.

V0_8_80 changes:
- Generated GUI iOS focused extractor now prefers 7-Zip when available.
- Generated extractor uses recursive include patterns matching prior successful tools.
- Adds Metadata/CoreSpotlight variants.
- Adds ios_zip_entry_probe.csv diagnostic sample output.
- Generates store inventory from staged files after extraction.
- Explicit iOS no-match is now a diagnostic empty result, not an application crash.
- Manual, quick start, troubleshooting, and release notes updated.

Validation performed here:
- src/app/app_runner.cpp syntax check with g++ -std=c++20 -fsyntax-only passed.
- Version references updated to 0.8.80.

Not validated here:
- Windows/MSVC build.
- PowerShell execution of generated focused extractor.
- Full GUI iOS FFS ZIP run.
- Full APFS external comparison.

Expected V0_8_80 success indicators:
- logs/ios_focused_zip_extract.log contains using_extractor=7z:<path> if 7-Zip is available.
- logs/ios_focused_zip_extract_7z.log exists.
- ios_zip_entry_probe.csv exists.
- ios_input_store_entry_inventory.csv contains store.db/.store.db rows if present in source ZIP.
- store_inventory.csv and store_selection.csv become nonzero if staging succeeds.

## V0_8_81_BUILD_VALIDATION_NOTES.txt

Vestigant Spotlight V0_8_81 build/validation notes

Reviewed uploaded continuation artifacts:
- V0_8_79_build.log and V0_8_80_build.log both show Windows/MSVC build complete and binary version checks for their respective versions.
- The old Upload_Thin_iOS_GUI_EmptyTab_Check.zip still represents the earlier failure category: generic ZIP staging or no iOS stores, with stores=0 and raw_records=0.
- The latest standalone V0_8_80 run artifacts show the focused iOS ZIP path working: `using_extractor=7z`, `matched_entries=1942`, `store_db_entries=10`, and successful native parsing.
- The V0_8_80 run parsed stores=10 with raw_records=75438, raw_key_values=0, raw_date_candidates=75438.
- Enrichment completed with artifacts=37719, timeline=75438, usage=0.

Issue classification:
- V0_8_78: GUI/source routing / generic ZIP staging.
- V0_8_79: focused iOS extraction reached, but .NET ZIP enumeration did not match entries.
- V0_8_80: focused 7-Zip extraction and Store-V2 parsing work. Remaining issue is iOS value/string-probe coverage and GUI review clarity.

V0_8_81 changes:
- Added FullValues-mode fallback raw string probes for likely iOS CoreSpotlight Store-V2 records when item metadata is empty.
- Added iOS decode-fallback probe retention before raw record insertion.
- Added opened-case iOS counts/status text to the iOS Investigation View.
- Updated version to 0.8.81 and added docs/release/validation files.

Validation performed here:
- Source text patch applied cleanly.
- Version references updated to 0.8.81 for app_info.cpp, CMakeLists.txt, VERSION, VERSION.txt.
- `g++ -std=c++20 -Isrc -fsyntax-only src/parsers/native_storedb_parser.cpp` passed in the Linux sandbox.

Not validated here:
- Windows/MSVC full build; the Win32 GUI file is Windows-only and could not be compiled in this Linux sandbox.
- GUI runtime on Windows.
- Full iOS FFS ZIP run against the 39 GiB source ZIP.
- APFS/AFF4 external comparison.

## V0_8_74_NOTES.txt

Vestigant Spotlight v0.8.74

Focused fixes after V0.8.73 review:
- iOS: when profile=ios and --full-scan is used, parser selection now selects all valid store.db/.store.db candidates rather than one primary per group. This is intended for forensic discovery where duplicate/alternate iOS CoreSpotlight stores should be parsed and compared, not silently skipped.
- iOS: Run-IosCoreSpotlightFocusedZip.ps1 writes ios_input_store_entry_inventory.csv before CLI execution so thin uploads show how many store.db/.store.db entries existed in the input ZIP/folder before CLI discovery.
- APFS: resource-fork stream copy and decmpfs reconstruction attempt caps increased from 12,000 to 30,000 to reduce cap-driven exclusion of late Cache .txt ResourceFork targets.
- Roadmap: added docs/FORENSIC_ROADMAP_MACOS_IOS_V0_8_74.md.
