...
# V1.6.41.1 New Chat Continuation

Provide `VestigantSpotlightInv_V1_6_41.zip`, `V1_6_41_build.log`, and the V1.6.41.1 macOS zipped Spotlight thin upload. Validation targets: `native_path_probe_artifacts_updated`, `native_basename_probe_artifacts_updated`, and reduced GUI `------NONAME------` where candidates exist.

# V1.6.41.1 New Chat Continuation

Provide `VestigantSpotlightInv_V1_6_41.zip`, `V1_6_41_build.log`, and the macOS zipped Spotlight thin upload. The specific validation target is external dbStr map loading for macOS Store-V2 and resulting GUI display-name/path improvement.

# V1.6.41.1 New Chat Continuation

Provide `VestigantSpotlightInv_V1_6_41.zip`, `V1_6_41_build.log`, and the next macOS zipped Spotlight thin upload. The specific issue under test is whether native path probe candidates now replace `------NONAME------` in GUI review rows.
...
# New Chat Continuation Guide - V1.6.41.1

Continue from V1.6.41.1. The next validation target is the V1.6.41.1 iOS thin upload. Validate active-comparison reconciliation, Missing-from-FFS lead-only wording, and CoreDuet interactionC parser checks. Do not treat Missing-from-FFS rows as deletion proof.
...
Use the V1.6.41.1 ZIP as the source of truth. Validate build log and thin upload before further changes. Track whether GUI source changes compile under MSVC and whether active filesystem/CoreDuet validation samples remain PASS.

## V1.6.41.1 continuation

Use V1.6.41.1 to retest the macOS `.Spotlight-V100` folder. Expected status includes `native_kv_persistence_macos_storev2`, not `native_kv_persistence_filtered` or iOS CoreSpotlight compact wording.
